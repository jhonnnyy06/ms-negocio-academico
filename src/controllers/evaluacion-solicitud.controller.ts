...
export class EvaluacionSolicitudController {
  ...
  async create(
    ...
    if (evalSolicitud) {
      ...
      <a href="${Keys.enlace}/${(await evalSolicitud).id
  ...
  async updateById(
    ...
    if (evaluacionSolicitud.respuesta == 3) {
      ...
      const solicitud = await this.solictudRepository.findOne({
        where: {
          id: evaluacionSolicitud.id_solicitud,
        },
      });

      const claveCifrada = '61b13b65';
      const crearUsaurioJurado = await this.usuarioJuradoRepository.create({
        id_jurado: jurado?.id,
  ...
  @patch('/recordatorio-solicitud/{id}')
  @response(204, {
    description: 'EvaluacionSolicitud PATCH success',
  })
  async ById(
    ...
    const datos = new NotificacionCorreo();
    datos.asunto = 'Recordatorio Evaluacion Solicitud';
    if (jurado?.correo) {
      datos.destinatario = jurado?.correo;
    }
    datos.mensaje = `Hola ${jurado?.nombreCompleto} le recordamos que usted ha sido elegido
        como posible evaluador, para confirmar o rechazar vaya al siguiente enlace
        <a href="${Keys.enlace}/${evaluacionSolicitud.id}">Confirmar Respuesta</a>`;
    this.notificaciones.EnviarCorreo(datos);
  }
}
